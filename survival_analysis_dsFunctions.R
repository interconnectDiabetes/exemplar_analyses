...
do_REM <- function(coeffs, s_err, labels, fmla, out_family, variable, ref_table){
	...
	if (out_family == 'gaussian') {
		...
		text(usr[1], usr[4], paste0(gsub(paste0(ref_table,"\\$"),"", deparse(fmla)),collapse="\n"), adj = c( 0, 1 ),cex=0.75)
		text(usr[1], usr[3], paste0(ref_table,variable), adj = c( 0, 0 ),cex=0.75)
	}
...
createModelFormula <- function(study, data_table, outcome, exposure, covariate_list, interaction_term = NULL, type = "survival") {
...
tunedLexisB <- function(ref_table_in, my_outcome_in, my_exit_col_in, study, new_table) {
  ...
  studyName = names(study)
	...
	datashield.assign(symbol = 'DANGER.nfilter.tab', value = quote(c(0.1)), opal = study)
	datashield.assign(symbol = 'DANGER.nfilter.glm', value = quote(c(0.1)), opal = study)
	idColString = paste0(ref_table_in, '$', 'ID')
	entryColString = paste0(ref_table_in, '$', 'newStartDate')
	...
	ds.lexis.b(data=ref_table_in, intervalWidth = interval_width, idCol = idColString, entryCol = entryColString, 
		exitCol = exitColString, statusCol = statusColString, expandDF = new_table, datasources = study)
	...
	for (k in 1:length(my_outcome)){
		...
		for (j in 1:length(my_exposure)){
			...
			for(i in 1:length(studies)) {
				...
				fmla <- createModelFormula(studies[i], ref_table, my_outcome[k], my_exposure[j], my_covariate, type = "survival")
				print(studies[i])
				print(fmla)
			...
			fmla <- as.formula(paste(paste0(ref_table,'$CENSOR')," ~ ", '0', '+', 'tid.f', '+', paste0(c(paste0(ref_table, '$',my_exposure[j]), paste0(ref_table, '$',my_covariate)), collapse= "+")))
			#meta analysis here
			for (n in 1:length(variables)){
...
extractExposureCoefficient <- function(outcome, exposure, data_table, study) {
  ...
  est_value = data_table[which(data_table$study == study & data_table$outcome == outcome & data_table$cov == exposure),]
  return(est_value$Estimate)
}
...
extractExposureCoefficientGroup <- function(outcome, exposure, data_table, studies = opals){
  ...
  # returns the exponentiated list
