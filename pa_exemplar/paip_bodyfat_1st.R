...
# my_exp_1 = c('MOD_VIG_filt', 'LTPA_DUR_filt', 'LTPA_EE_filt')
...
#     if (my_exp_1[i] == 'LTPA_DUR_filt' & study_names[o] == 'GECKO'){
#       next
#     }
...
mypath <- file.path('~','plots','model_1_bf_vig.svg')
svg(filename=mypath, 
...
for (k in 1:length(my_outcome)){
  ...
  for (j in 1:length(my_exposure)){
    ...
    for(i in 1:length(opals)) {
      ...
      if (my_exposure[j] == 'LTPA_DUR_filt_3' & study_names[i] == 'GECKO'){
...
mypath <- file.path('~','plots','model_2_bf_vig.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
for (k in 1:length(my_outcome)){
  ...
  mypath <- file.path('~','plots',paste('model_5_bf_vig_', k, '.svg',sep=''))
