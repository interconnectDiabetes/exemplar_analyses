## Analysis script for first trimester PAIP analysis focused on ponderal index in
...
source("creds/pa_exemplar_creds_3_pon_ind.R")
setwd("~")
...
# my_exp_1 = c('MOD_VIG_3_filt', 'LTPA_DUR_3_filt')
# my_outcome_1 = c('PON_INDEX')
...
#     if (my_exp_1[i] == 'LTPA_DUR_3_filt' & study_names[o] == 'GECKO'){
#       next
#     }
...
mypath <- file.path('~','plots','model_1_pi_3rd_vig.svg')
svg(filename=mypath, 
...
# my_outcome_2 = c('BIRTH_WEIGHT', 'MACROSOMIA', 'BIRTH_WEIGHT_LGA', 'PON_INDEX')
...
#     # we skip LTPA for GECKO
#     if (my_exp_2[i] == 'LTPA_DUR_3_filt' & study_names[o] == 'GECKO'){
#       next
#     }
...
mypath <- file.path('~','plots','model_2_pi_3rd_vig.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
################## MODEL 3 starts here ########################
#
#  Mediation analysis looks at 4 steps (more information here:
#  http://davidakenny.net/cm/mediate.htm). The steps are:
#
# 1. If there is no overall association between the exposure and outcome
#    then no need to run mediation analysis (i.e. model 2 not significant)
# 2. Run model 2 but with mediator as the outcome. Is the exposure coefficient
#    significant? ie is the exposure correlated with the mediator?
# 3. Run model 2 with the mediator included as the exposure (the true exposure
#    should still be included as a covariate). Is the mediator coefficient significant
#    ? ie does the mediator effect the outcome?
# 4. Run model 2 with the mediator as a covariate. Is the exposure coefficient zero?
#    If yes, then full mediation, if no and 1-3 are true then partial mediation.
#
# 

#### Step 1 - Normally not needed as just model 2


#### Step 2 - this is Model 2 but with the mediator as the outcome
...
mypath <- file.path('~','plots','model_3_2_PI3rd_vig.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
#### Step 3 - Run model 2 with the mediator included as the exposure (the true exposure
#    should still be included as a covariate)
# Need to run on a per exposure basis

my_exposure = 'MATERNAL_BMI'
my_outcome = c('PON_INDEX')
my_covariate = c('GESTATIONAL_AGE', 'SEX', 'PARITY', 'MATERNAL_AGE', 'SMOKING',
                 'ALCOHOL', 'MATERNAL_EDU', 'ETHNICITY', 'LTPA_EE_3_filt')

mypath <- file.path('~','plots','model_3_3_PI_EE.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
mypath <- file.path('~','plots','model_3_4_PI.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
#Store results
dev.off()
model_3_4_1_all <- study_regs
model_3_4_1_REM <- REM_results
...
mypath <- file.path('~','plots','model_4_2_GDM.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
mypath <- file.path('~','plots','model_4_3_GDM_EE.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
#Store results
dev.off()
model_4_4_1all <- study_regs
model_4_4_1REM <- REM_results
...
for (k in 1:length(my_outcome)){
  ...
  mypath <- file.path('~','plots',paste('model_5_pi_3rd_vig_', k, '.svg',sep=''))
  svg(filename=mypath, 
...
for (k in 1:length(my_outcome)){
  ...
  #set up plots here because there are plots for interaction terms too
  mypath <- file.path('~','plots',paste('model_6_pi_3rd_vig_', k, '.svg',sep=''))
  svg(filename=mypath, 
      width=4 * number_of_interactions, 
      height=3 * length(my_outcome), 
      pointsize=10)
...
mypath <- file.path('~','plots','model_6_0.svg')
...
mypath <- file.path('~','plots','model_6_1.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
mypath <- file.path('~','plots','model_6_2.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
for (k in 1:length(my_outcome)){
  ...
  #set up plots here because there are plots for interaction terms too
  mypath <- file.path('~','plots',paste('model_7_pi_3rd_vig_', k, '.svg',sep=''))
  svg(filename=mypath, 
      width=4 * number_of_interactions, 
      height=3 * length(my_outcome), 
      pointsize=10)
...
# |  \/  |         | |    | | |___  /          /  ___| |           | |       
# | .  . | ___   __| | ___| |    / /   ______  \ `--.| |_ _ __ __ _| |_ __ _ 
# | |\/| |/ _ \ / _` |/ _ \ |   / /   |______|  `--. \ __| '__/ _` | __/ _` |
# | |  | | (_) | (_| |  __/ | ./ /             /\__/ / |_| | | (_| | || (_| |
# \_|  |_/\___/ \__,_|\___|_| \_/              \____/ \__|_|  \__,_|\__\__,_|

# Ethnicity stratification code
ds.subset(x = 'E4', subset = 'E4_7_0', logicalOperator = 'ETHNICITY==', threshold = 1)
ds.subset(x = 'E4', subset = 'E4_7_1', logicalOperator = 'ETHNICITY==', threshold = 2)
ds.subset(x = 'E4', subset = 'E4_7_2', logicalOperator = 'ETHNICITY==', threshold = 3)

my_exposure = c('LTPA_EE_3_filt')
...
mypath <- file.path('~','plots','model_7_0.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
#Store results
dev.off()
model_7_0_all <- study_regs
model_7_0_REM <- REM_results
...
mypath <- file.path('~','plots','model_7_1.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
#Store results
dev.off()
model_7_1_all <- study_regs
model_7_1_REM <- REM_results
...
mypath <- file.path('~','plots','model_7_2.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
#Store results
dev.off()
model_7_2_all <- study_regs
model_7_2_REM <- REM_results
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#
# SPECIAL EXCEPTION FOR USING VIG_FILT - HSS HAVE SINGULAR
# MATRIX PROBLEM WITH GDM
...
for (k in 1:length(my_outcome)){
  ...
  #set up plots here because there are plots for interaction terms too
  mypath <- file.path('~','plots',paste('model_8_pi_3rd_vig_', k, '.svg',sep=''))
  svg(filename=mypath, 
      width=4 * number_of_interactions, 
      height=3 * length(my_outcome), 
      pointsize=10)
...
# |  \/  |         | |    | | |  _  |          /  ___| |           | |       
# | .  . | ___   __| | ___| |  \ V /   ______  \ `--.| |_ _ __ __ _| |_ __ _ 
# | |\/| |/ _ \ / _` |/ _ \ |  / _ \  |______|  `--. \ __| '__/ _` | __/ _` |
# | |  | | (_) | (_| |  __/ | | |_| |          /\__/ / |_| | | (_| | || (_| |
# \_|  |_/\___/ \__,_|\___|_| \_____/          \____/ \__|_|  \__,_|\__\__,_|
#                                    

# GDM stratification code
ds.subset(x = 'E4', subset = 'E4_8_0', logicalOperator = 'GDM==', threshold = 0)
ds.subset(x = 'E4', subset = 'E4_8_1', logicalOperator = 'GDM==', threshold = 1)
...
mypath <- file.path('~','plots','model_8_0.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
#Store results
dev.off()
model_8_0_all <- study_regs
model_8_0_REM <- REM_results
...
mypath <- file.path('~','plots','model_8_1.svg')
svg(filename=mypath, 
    width=4 * length(my_exposure), 
    height=3 * length(my_outcome), 
    pointsize=10)
...
#Store results
dev.off()
model_8_1_all <- study_regs
model_8_1_REM <- REM_results
