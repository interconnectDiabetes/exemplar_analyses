...
mypath <- file.path('~','plots','model_1_bf_3rd.png')
...
mypath <- file.path('~','plots','model_2_bf_3rd.png')
png(file=mypath, width = 1260*3, height = 940*3, res = 300)
par(mar=c(5,3,2,2)+0.1)
par(mfrow=c(3,3))
...
for (k in 1:length(my_outcome)){
  ...
  for (j in 1:length(my_exposure)){
    ...
    #meta analysis here
    for (n in 1:length(variables)){
      mypath <- file.path('~','plots',paste('model_5_bf_3rd',j,'_',k,'_',n, '.png',sep=''))
      png(file=mypath, width = 1260, height = 940)
      REM_results[[paste(c(my_outcome[k], my_exposure[j],my_covariate, variables[n],'REM'),collapse="_")]]  <- do_REM(estimates[,n], s_errors[,n], labels, fmla,out_family = outcome_family, variable = variables[n])
      dev.off()
    }
...
for (k in 1:length(my_outcome)){
  ...
  for (j in 1:length(my_exposure)){
    ...
    #meta analysis here
    for (n in 1:length(variables)){
      mypath <- file.path('~','plots',paste('model_6_bf_3rd',j,'_',k,'_',n, '.png',sep=''))
      png(file=mypath, width = 1260, height = 940)
      REM_results[[paste(c(my_outcome[k], my_exposure[j],my_covariate, variables[n],'REM'),collapse="_")]]  <- do_REM(estimates[,n], s_errors[,n], labels, fmla,out_family = outcome_family, variable = variables[n])
      dev.off()
    }
...
for (k in 1:length(my_outcome)){
  ...
  for (j in 1:length(my_exposure)){
    ...
    #meta analysis here
    for (n in 1:length(variables)){
      mypath <- file.path('~','plots',paste('model_7_bf_3rd',j,'_',k,'_',n, '.png',sep=''))
      png(file=mypath, width = 1260, height = 940)
      REM_results[[paste(c(my_outcome[k], my_exposure[j],my_covariate, variables[n],'REM'),collapse="_")]]  <- do_REM(estimates[,n], s_errors[,n], labels, fmla,out_family = outcome_family, variable = variables[n])
      dev.off()
    }
...
for (k in 1:length(my_outcome)){
  ...
  for (j in 1:length(my_exposure)){
    ...
    #meta analysis here
    for (n in 1:length(variables)){
      mypath <- file.path('~','plots',paste('model_8_bf_3rd',j,'_',k,'_',n, '.png',sep=''))
      png(file=mypath, width = 1260, height = 940)
      REM_results[[paste(c(my_outcome[k], my_exposure[j],my_covariate, variables[n],'REM'),collapse="_")]]  <- do_REM(estimates[,n], s_errors[,n], labels, fmla,out_family = outcome_family, variable = variables[n])
      dev.off()
    }
