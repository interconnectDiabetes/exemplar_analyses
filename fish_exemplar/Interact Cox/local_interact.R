...
cox_list <- function(DF) {
  ...
  model <- summary(coxph(Surv(start, end, CASE_OBJ)~TOTAL+AGE_BASE+EDUCATION+SMOKING+MI+STROKE+HYPERTENSION, DF, robust = TRUE, ties = 'breslow'))
  return(model)
...
dev.off()
svg(filename='/home/l_trpb2/test.svg', 
    width=4 * 1, 
    height=3 * 1, 
    pointsize=10)
