...
hmohiv$entdate2 <- as.numeric(as.Date(hmohiv$entdate,"%m/%d/%Y"))
ent_min <- min(hmohiv$entdate2)
hmohiv$entdate2 <- hmohiv$entdate2 - ent_min
hmohiv$entdate <- hmohiv$entdate2

hmohiv$enddate2 <- as.numeric(as.Date(hmohiv$enddate,"%m/%d/%Y"))
hmohiv$enddate2 <- hmohiv$enddate2 - ent_min
hmohiv$enddate <- hmohiv$enddate2

hmohiv <- hmohiv[-c(8:9)]

write.csv(hmohiv,"U:/___Personal___/hmohiv.csv")

server <- c('study0')
url <- c('https://shrine0.mrc-epid.cam.ac.uk:8443')
